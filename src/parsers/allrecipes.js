...
const getMetaInfo = ($, header) =>
  $(".recipe-info-section")
    .find(".recipe-meta-item")
    .map((_, element) => {
    ...
    .get()
    .find((section) =>
      section.header.toLowerCase().includes(header.toLowerCase())
    );
...
const parse = async (source, notes, rating) => {
  ...
  const time = $(".two-subcol-content-wrapper")
    .first()
    .find(".recipe-meta-item")
    .map((_, element) => {
      const select = Cheerio.load($(element).html());
      const label = select(".recipe-meta-item-header")
        .text()
        .trim()
        .replace(":", "");
