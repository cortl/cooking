...
const parse = async (source, notes, rating) => {
  ...
  const items = $(".ingredient")
    ...
    .get();
  const instructions = $(".structured-project__steps")
    .find("ol")
    .find("li")
  ...
  const image = imageUrl
    ...
    : "";
  const servings = $(".project-meta__results-container")
    .text()
    .split(" ")
    .map((word) => parseInt(word))
    .find(Number.isInteger);

  const time = $(".project-meta__times-container")
    .find(".loc")
    .map((i, element) => {
      const label = $(element)
        .find(".meta-text__label")
        .text()
        .trim()
        .replace(":", "");
